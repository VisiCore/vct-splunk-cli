...
Reads are always available. Writes exist too, but only through the gated path in
:mod:`vct_splunk.commands.write` -- this client sends whatever it is told, the
same way :class:`~vct_splunk.api.client.SplunkClient` does; the opt-in gate and
resource allowlist live one layer up.
"""
...
class AcsClient:
    ...
    caller opted in; that gate is :func:`vct_splunk.commands.write.refuse_cloud_write`,
    one layer up.
    """
    ...
    def write(self, method: str, path: str, json_body: Any | None = None) -> Any:
        ...
        Mirrors :meth:`vct_splunk.api.client.SplunkClient.write`: the caller
        (an ACS operation function) is trusted to have already decided this
        mutation is allowed to run; this method only decides whether to send it.
        """
